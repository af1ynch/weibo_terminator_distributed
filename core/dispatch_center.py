...
class Dispatcher(object):
    ...
    def __init__(self, uid, filter_flag=0, update_cookies=False):
        # self.mode = mode
    ...
    def _init_accounts_cookies(self):
        ...
        if self.update_cookies:
            for account in accounts:
                ...
                get_cookie_from_network(account['un'], account['pw'])
    ...
    def _init_accounts(self):
        ...
        try:
            ...
            print('----------- detected {} accounts, wei bo terminator will using all accounts to scrap '
    ...
    def _init_single_mode(self):
        ...
        i = 1
        while True:
            if not scraper.has_crawled_queue.set_is_member(scraper.user_id):
                result = scraper.crawl()
                if result:
                    scraper.has_crawled_queue.set_add(scraper.user_id)
                    if not scraper.uid_queue.set_empty():
                        new_uuid = scraper.uid_queue.set_pop().decode()
                        if scraper.has_crawled_queue.set_is_member(new_uuid):
                            continue
                        else:
                            scraper.user_id = new_uuid
                    else:
                        print("There is no uid in set")
                        break
                else:
                    if i >= len(self.all_accounts):
                        ...
                        self._init_accounts_cookies()
                        self._init_accounts()
                        scraper = WeiBoScraper(using_account=self.all_accounts[0], uuid=scraper.user_id,
                                               filter_flag=self.filter_flag)
                    else:
                        scraper.switch_account(self.all_accounts[i])
                        print('account {} being banned or error wei bo is none for current user id, switch to {}..'.
                              format(self.all_accounts[i - 1], self.all_accounts[i]))
                        i += 1
            else:
                scraper.user_id = scraper.uid_queue.set_pop().decode()
